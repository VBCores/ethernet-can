# Multiple boards: browser configuration
...
2. Find the computer IP with `ip -4 -br address` and reserve it on the router. Replace `192.168.2.2` with this address in **both** [board1_host_config.json](board1_host_config.json) and [board2_host_config.json](board2_host_config.json). Interfaces are already separate: first board bus0 → `vcan1.0`, second board bus0 → `vcan2.0`.
3. Copy [board1_sd_config.json](board1_sd_config.json) to the first board's FAT/FAT32 SD as `config.json`; copy [board2_sd_config.json](board2_sd_config.json) to the second card under the same name. Safely eject the cards, insert with board power off, then power both boards.
4. Check addresses:
...
5. Stop the service: `sudo systemctl stop ethernet-can.service`. Open `http://ethernetcan-1.local/panel` and `http://ethernetcan-2.local/panel` in turn. Paste [runtime_config.json](runtime_config.json) into the panel JSON field, replace `data_plane.host_ip` with the same computer IP, check CAN bitrates and click Apply. The example enables only bus0. For classic CAN set `data_kbit: 0`. Wait for `fdcan.config_applied=true`.
...
Host JSON has no `fdcan`: the client waits for each board's saved settings. All boards must use the same computer IP, but distinct board IPs and interface names. When changing enabled buses, update interface maps; restart the service after changing classic/FD mode.
