# Две платы: настройка с компьютера и через браузер
...
2. Найдите IP компьютера командой `ip -4 -br address` и закрепите его на роутере. Замените `192.168.2.2` этим адресом в **обоих** файлах: [host_managed_host_config.json](host_managed_host_config.json) и [board_managed_host_config.json](board_managed_host_config.json). Интерфейсы уже разделены: bus0 первой платы → `vcan1.0`, второй → `vcan2.0`.
3. На отдельную FAT/FAT32 SD первой платы скопируйте [host_managed_sd_config.json](host_managed_sd_config.json) как `config.json`; на SD второй — [board_managed_sd_config.json](board_managed_sd_config.json) с тем же именем. Безопасно извлеките карты, вставьте в выключенные платы и включите их.
4. Проверьте адреса:
...
5. Остановите службу: `sudo systemctl stop ethernet-can.service`. У первой платы настройки задаёт секция `fdcan` в её host JSON: проверьте скорости там. Откройте `http://ethernetcan-2.local/panel` для второй платы. В поле JSON панели вставьте [runtime_config.json](runtime_config.json), замените `data_plane.host_ip` тем же IP компьютера, проверьте скорости CAN и нажмите Apply. Пример включает только bus0; другие шины выключены. Для classic CAN установите `data_kbit: 0`. Дождитесь `fdcan.config_applied=true`.
...
Клиент настраивает первую плату через REST и ждёт сохранённых настроек второй. Все платы должны использовать один IP компьютера, но разные IP плат и разные имена интерфейсов. При изменении включённых шин согласуйте карты интерфейсов; после смены classic/FD перезапустите службу.
